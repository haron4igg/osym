...
public class Image2D extends AbstractImage {
    ...
    public Image2D(String name, int width, int heigth, int scopeX, int scopeY) {
        ...
        imageMas = new short[heigth + 1][width + 1];
    ...
    public void draw(Graphics g, int x, int y) {
        ...
        for (int i = 0; i < myHeight + 1; i++) {
            for (int j = 0; j < myWidth + 1; j++) {
                ...
                if (imageMas[i][j] != -1) {
                    g.setColor(new Color(imageMas[i][j], imageMas[i][j], imageMas[i][j]));
